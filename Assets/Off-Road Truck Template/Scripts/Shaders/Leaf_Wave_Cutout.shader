...
Shader "Off-Road Truck Kit/Leaf Wave Cutout" {
Properties {
    ...
    _WaveMultiplier ("_Multi1", Range (0, 10)) = 1.0
    _WindMultiplier ("_Multi1", Range (0, 3)) = 1.0
}
...
void vert (inout appdata_full v) {
    ...
    float4 waves;
    waves = v.vertex.x * _waveXSize*_WaveMultiplier;
    waves += v.vertex.z * _waveZSize*_WaveMultiplier;
    ...
    float waveAmount = v.texcoord.y * (v.color.a + _ShakeBending)*_WindMultiplier;
    s *= waveAmount;
